...
def show_samples(data_path, row=3, col=1):
    ...
    for r in range(row):
        h = None
        for c in range(col):
            image = imresize(image=imread(files[i][0]), size=(50, 50))
            image = normalize_array(pil2array(image))
            i += 1
            if h is None:
                h = image.copy()
                h = np.hstack((h, np.zeros((50, 1))))
            else:
                h = np.hstack((h, image, np.zeros((50, 1))))
...
if __name__ == '__main__':
    train_data, train_labels, test_data, test_labels, label_map = create_handwritten_dataset("datasetss")
